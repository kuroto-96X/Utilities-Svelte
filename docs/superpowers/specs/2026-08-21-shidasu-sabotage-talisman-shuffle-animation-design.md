...
実装調査の過程で、デバッグ画面(`/admin/shidasu-debug`)の`itemBadges`スニペットには、本編の`+page.svelte`にある`talismanHidden`の常設表示(斜めストライプ背景・「？？？」表示)自体が実装されていないことが判明した(`talismanSealed`の常設表示のみ対応済み)。これは今回のスコープ外で発生していた既存のギャップだが、今回`talismanShuffle`の発動演出を目視確認できるようにするため、デバッグ画面側にも本編と同じ`talismanHidden`常設表示+今回の発動演出の両方を実装する。
...
- デバッグ画面(`/admin/shidasu-debug`)で「護符並び替え」ボタンを発動し、所持護符バッジ全体が同時にフラッシュ+シェイクし、発動後は「？？？」表示に切り替わることを目視確認する。デバッグ画面には既に護符所持チェックリスト機能があるため、事前に複数護符を所持させてから確認する。
