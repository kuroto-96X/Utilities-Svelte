...
- 押下時、`deckComposition = standardDeckComposition()`を実行したうえで、リセットされたデッキから場札・山札を配り直す(新しいウェーブを開始する)
- 配り直しは`startWave(params, 0, 0, [], deckComposition)`のように**護符リストに空配列を渡して**実行する。これにより、永劫(ワイルド追加)・豊穣(ランダム変換)・暗雲(場札増加)・約束(山札並べ替え)・架橋(階段等の緩和)といった、`startWave`内で`items`を直接参照するウェーブ開始時効果が一切発動しない。デッキリセットの目的(永続系護符による変化を標準構成に戻すこと)を、配り直した直後の護符効果によって台無しにしないための措置
- プレイ・山札めくり時の護符効果(現在チェックされている`items`を使う`playCard`/`drawStock`)には影響しない。あくまで配り直しの瞬間だけ護符を無視する
- `lastSnapshot`(1.1)はクリアする(場札・山札が実際に変わるため、直前操作の「元に戻す」はデッキリセット後は無効になる)
...
4. デバッグサンドボックスで「デッキリセット」を押すと、標準デッキ構成から即座に場札・山札が配り直され(永続系護符によるデッキ変化が解消される)、その配り直し自体では永劫・豊穣・暗雲などの護符効果が発動しない
