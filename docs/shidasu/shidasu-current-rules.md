...
- 場札(tableau): `layout.cols`(既定**7**)列×実配布行数を裏向き山から配る。実配布行数は`layout.rows`(既定**5**)+`extraTableauRows`(星`moon`の`initialExtraTableauRows: -1`などランを通じて累積する加算値。現状は増加させる護符・天啓は存在しない)。
- 基準カード(foundation): 場札配布後の山から1枚。
- 山札(stock): 残り枚数(標準52枚から場札配布分・基準カード分を引いた枚数)。
- 標準デッキ(`standardDeckComposition`)は常に**ワイルドカードを含まない52枚**。ワイルドは永劫・豊穣等の護符でのみ供給される(6.7節参照)。
...
ワイルドカードは、その時点で必要とされるランク・スートそのものの代役となるだけで、それ以外の特別ルールは無い。標準デッキにはワイルドが含まれず、永劫・豊穣・静寂などの護符によって供給される。
...
waveSlot3の星(現行6種全て`sabotage: { kind: 'all' }`)は、上記の取得可否制限・得点ロックに加えて、一定ターンごとに能動的にゲーム状態を崩す「妨害行動」も持つ。Wave開始時に`SABOTAGE_POOL`(22種)から1つが抽選され(`wave.pendingSabotageId`・`wave.sabotageTurnsRemaining`)、以後「1ターンごとに残りターンを1減らし、0になったら発動して次の妨害を再抽選する(直前と同じ候補が連続する可能性もある)」というサイクルを繰り返す。ターンの定義は4.6節の山札めくりと同じで、`playCard`は常にカウントし、`drawStock`はめくった札がパターン(同スート/同色/階段/交互のいずれか)・チェーン・コンボを継続させた場合にカウントする(素朴(naive)の所持有無は無関係。naiveは継続時に得点計算するかどうかにのみ影響する)。効果の実発動は`applyPlayCard`/`applyDrawStock`/`applyStuckCheck`(RunState層)側の`triggerSabotage`が行い、`WaveState`層の`playCard`/`drawStock`自体はターンのカウントダウンのみを行う。

| 妨害 | ターゲット | 発動間隔(ターン) | 効果 |
|---|---|---|---|
| 大量放出(stockPurge) | 山札 | 6 | 山札の上から5枚を捨て札に置く |
| 一列戻し(columnReturn) | 場札 | 6 | ランダムな1列を山札に戻し、シャッフル後同じ列に裏向きで再配布する |
| 強制清算(chainSettle) | チェーン | 8 | チェーンを全て捨て札に送り、山札から1枚めくって新しいチェーンにする。コンボも0にする |
| 強制小休止(comboBreather) | コンボ | 5 | チェーンはそのまま、コンボ数だけ0にする |
| 護符封印(talismanSeal) | 護符 | 5 | 所持護符を1つ選び、次の妨害発動まで効果を無効化する |
| 秘儀封印(riteSeal) | 秘儀 | 5 | 所持秘儀を1つ選び、次の妨害発動まで使用を禁止する |
| 天啓・神託封印(revelationOracleSeal) | 天啓・神託 | 5 | 天啓または神託を1つ選ぶ(天啓は次の妨害発動まで使用禁止、神託は対象役のボーナス計算を一時的に抑制) |
| レリック没収(relicConfiscate) | レリック | 7 | 所持レリックを1つ選び、完全に失わせる |
| 一枚没収(tableauCardToDiscard) | 捨て札 | 4 | 場札からランダムに1枚選び捨て札に送る |
| 通貨没収(currencyConfiscate) | 資産(星片) | 6 | 所持する星片を5減らす(0未満にはしない) |
| 役封印(roleSeal) | 役ステータス | 6 | ランダムな2役を選び、次の妨害発動までそれらのボーナスを無効化する |
| 少量放出(stockPurgeSmall) | 山札 | 4 | 山札の上から2枚を捨て札に置く |
| 山札攪拌(stockShuffle) | 山札 | 5 | 山札の順序をランダムに並び替える(枚数は変わらない) |
| 総戻し(tableauFullReturn) | 場札 | 8 | 場札全体を山札に戻し、シャッフル後同じ配分で再配布する |
| 総入れ替え(tableauShuffle) | 場札 | 6 | 場札の中身を列をまたいでランダムに再配置する(山札には触れない) |
| チェーン部分放棄(chainPartialDiscard) | チェーン | 5 | チェーンの先頭(最古)から2枚を捨て札に送る(コンボはそのまま維持)。ただしチェーンは空にせず、必ず最低1枚(現在の基準カード)は残す |
| チェーン入れ替え(chainShuffle) | チェーン | 6 | チェーンをシャッフルし、新しい末尾を基準カードにする |
| コンボ削減(comboReduce) | コンボ | 5 | コンボ数を3減らす(0未満にはしない) |
| コンボ頭打ち(comboCap) | コンボ | 6 | 発動時点のコンボ数を上限として、次の妨害発動まで増加を止める |
| 護符没収(talismanConfiscate) | 護符 | 7 | 所持護符を1つ選び、完全に失わせる |
| 秘儀没収(riteConfiscate) | 秘儀 | 6 | 所持秘儀を1つ選び、効果を発動させずに消費させる |
| 秘儀強制発動(riteForceActivate) | 秘儀 | 6 | 使用可能(`canUseRite`を満たす)な秘儀を1つ選び、プレイヤーの操作を介さず即座に効果を発動させて消費する(使用可能な秘儀が無ければ不発) |

封印系(護符・秘儀・天啓/神託・役ステータス)は常に最大1件しか同時に成立しない(`WaveState.activeSeal`。新しい妨害が発動するたびに一旦`null`へリセットしてから、今回の効果が封印系ならあらためて設定される)。封印中でも所持表示自体は維持される。護符は効果適用時に除外され、秘儀・天啓は使用ボタンがdisabledになる。天啓・神託封印(revelationOracleSeal)で神託が選ばれた場合は使用ボタンには影響せず、代わりに対象役のオラクルレベルを一時的に1相当(未強化の基準値)まで下げてボーナスを抑制する(役封印(roleSeal)は対象2役のオラクルレベルを0扱いにしてボーナスを完全に無効化する、より強い効果)。いずれも`oracleLevel`の一時的な上書きとして実装され、`RunState.oracleLevels`自体は書き換えない。コンボ頭打ち(comboCap)も同じ`activeSeal`の仕組みを共有するが、対象は護符・秘儀・天啓/神託・役ではなくコンボ数そのものの上限値であり、UIの使用ボタンを無効化するのではなく`playCard`/`drawStock`のコンボ加算結果を数値としてクランプする点が他の封印系と異なる。プレイ中の画面には、星の制限説明の下に次に発動する妨害の名前と残りターン数(「次の妨害: {name}(あと{N}ターン)」)が表示される。
...
- モチーフは北欧ルーン文字(エルダー・フサルク)。全**24種**実装済み(`RITE_POOL`)。2026-08-11に一度削除したカード変換系9種は、同日中に別の効果として再設計・復元された(詳細は`docs/shidasu/done/shidasu-rite-redesign-candidates.md`を参照)。
- ウェーブを跨いで永続所持(所持上限**3**)。プレイ中に能動的に使用する消費アイテムで、`useRite`で1個消費して即時効果を発動する。一部(ゲボ・フェフ、ペルスロ・ラグズ・アンスズ)は使用条件(山札・捨て札の枚数、不足列・空列・山札切れの有無)がある。
- 詳細は`docs/shidasu/done/shidasu-higi-candidates.md`(実装済み)を参照。

### 7.3 天啓(③Revelation)

- モチーフは二十八宿。全28宿全てに効果実装済み(`REVELATION_POOL`)。天啓「虚」(2026-08-13)は所持レリックの付喪化(選んだ1つを進化させる)効果として再実装済み。天啓「鬼」(2026-08-13)は未所持レリックのランダム獲得効果として実装済み。
...
- トランプセット福袋(`packKind: 'cardSet'`): バラ売り無し、福袋限定(3-1/5-1/7-2パターン)。開けると23種類のセットジャンル(階段・同ランク・絵札・同スート・ロイヤル・フラッシュ・コンプリートラン・ペア・赤黒バランス・ワイルド)が重み付き抽選(重み=1/枚数、ワイルドのみ1/6)で提示され、選ぶと即座に`RunState.deckComposition`へ永続追加される(所持枠・スワップ処理は無い)。
- ショップリロール(`rerollShop`): バラ売り3枠+福袋2枠を一括で再抽選するボタン。売り切れ済みの枠も含めて全て新しい商品に入れ替わる(既存の`rollShop`を再利用、既に購入済みのアイテムには影響しない)。コストは通貨消費で、同一ショップ訪問中のリロール回数(`RunState.shopRerollCount`)に応じて`shop.rerollCostStep`(既定**5**)ずつ増額する(1回目5、2回目10、3回目15…)。次のショップに入るたびに0にリセットされる。
