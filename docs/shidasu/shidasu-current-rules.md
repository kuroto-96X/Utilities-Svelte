...
| 天啓・神託封印(revelationOracleSeal) | 天啓・神託 | 5 | 天啓または神託を1つ選び、次の妨害発動まで使用禁止にする |
...
封印系(護符・秘儀・天啓/神託・役ステータス)は常に最大1件しか同時に成立しない(`RunState.activeSeal`。新しい妨害が発動するたびに一旦`null`へリセットしてから、今回の効果が封印系ならあらためて設定される)。封印中でも所持表示自体は維持されるが、護符は効果適用時に除外され、秘儀・天啓・神託は使用ボタンがdisabledになり、役封印中はオラクルレベルを一時的に1相当まで下げてボーナスを止める。プレイ中の画面には、星の制限説明の下に次に発動する妨害の名前と残りターン数(「次の妨害: {name}(あと{N}ターン)」)が表示される。
