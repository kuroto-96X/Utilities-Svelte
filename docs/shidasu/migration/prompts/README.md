...
## フェーズ13 実行結果(1/2、共通パターン+カード移動、完了)

Web版の「表示専用スナップショット+ガードフラグ+完了時同期」という個別対策パターンを、`DisplaySync<T>`(ジェネリックな表示専用スナップショット+ガード+完了時同期)と`AnimationActivityTracker`(トークン登録方式による演出実行中フラグの集約)という再利用可能な仕組みに一般化した。カード移動(gather→move)をTweenで実装し、チェーンリセット演出に適用して先出しバグが起きないことをログで確認。設計ドキュメント(`13-animation-design.md`)に後続フェーズ(カード裏返し・妨害演出4種)が従うべき実装規約を記載済み。`dotnet build`/`dotnet test`成功、722件のテスト全て成功。GitHubリモートへpush済み。次はフェーズ13の2/2(カード裏返し+妨害演出4種)。

## フェーズ14 実行結果(完了)

画像生成ツールが使えないため、Godotのベクター描画機能(`Control._Draw()`/`GPUParticles2D`/シェーダー)でプログラム的にアセットを生成する方針を採用。既存のゲームプレイ用スクリプト(CardFace.cs等)は一切編集せず、新規ファイルのみで実装(フェーズ13との並行編集競合を回避)。

- カードデザイン: 52通り+ワイルド+裏面をベクター描画
- カテゴリ別モチーフ: カタログを精査した結果、**当初想定(護符=北欧ルーン)は誤りと判明**。実際は秘儀(Rite)24種が北欧ルーン(エルダー・フサルク)、護符(Talisman)133種は和風縁起物。天啓28種=二十八宿、神託10役=六十四卦、レリック10種=和風縁起物、妨害32種=9グループの障り意匠、スプレッド10種=大アルカナ、という対応表を確定し全件数を検証済み
- 背景・エフェクト: タイトル/プレイ/ショップ/全クリア/ゲームオーバーの5画面分の背景と、フェーズ13の演出用エフェクト素材(発光・警告ビネット・パーティクル散布)を実装
