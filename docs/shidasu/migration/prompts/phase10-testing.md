...
Web版はロジック層(`src/lib/game/shidasu/`)のほぼ全モジュールにvitestテストが対応しており、テストファイル21個・合計約10,700行(`engine.test.ts`だけで6,267行、約80個の`describe`ブロック)という高い網羅性を持つ。この網羅性をGodot版(GDScript)でも再現し、以後の変更(バランス調整・バグ修正)に対する回帰検知の土台を作る。
...
- Godot側のテスティングフレームワークとして **GUT(Gut Unit Test)** を導入する。GUTはGodotアドオン(`res://addons/gut/`)として組み込み、テストスクリプトは`GutTest`を継承したクラスに`test_`プレフィックスのメソッドを書く形式(`assert_eq`/`assert_true`/`assert_almost_eq`等のアサーションAPIを提供)。コマンドラインからのヘッドレス実行(`-s addons/gut/gut_cmdln.gd`相当)にも対応させ、CI相当の自動実行ができる状態を目指す。

## 作業内容

### 1. GUTの導入

- Godotプロジェクトに`gut`アドオンを追加し、プロジェクト設定でアドオンを有効化する。
- テストファイルの配置規約を決める(例: `res://tests/unit/`配下に、移植元のvitestファイル名に対応させた`test_*.gd`を1対1で配置)。
- コマンドラインからヘッドレスで全テストを実行できることを確認する(エディタを開かずに実行できる状態を必須とする。CIやフェーズ16のパッケージング前チェックで使うため)。

### 2. vitestテストケースのGUT形式への翻訳
...
5. **`engine.test.ts`全体の棚卸し**: 上記1〜4のテストで拾いきれない`engine.ts`固有のロジック(`startWave`/`playCard`/`drawStock`/`isStuck`/`resolveWaveEnd`/`beginRun`/ショップフロー関数群/`useRite`/`useRevelation`/`useOracle`/`sellItem`系等)を、`engine.test.ts`の`describe`ブロック一覧(345行目`startWave`から6247行目付近`賞金・僥倖・祝儀`まで、約80ブロック)を目視で洗い出し、対応するGUTテストが存在するか突き合わせる。抜けがあれば追加する。

各vitestの`describe`/`it`は、原則としてGUTの`test_`メソッド1つに1対1対応させる(粒度を大きくまとめ直さない。後からどのWeb版テストに対応するGUTテストか追跡できるようにするため)。`testHelpers.ts`(`card()`・`ctx()`ヘルパー)に相当する共通ヘルパースクリプトをGUT側にも用意し、テストごとに手組みのダミーオブジェクトを重複して書かないようにする。

### 3. 固定シードによるスモークテスト

- `createRng`相当のGodot側RNG実装(フェーズ2で方針決定済み)に対し、固定シードを与えてRun開始→全ステージ・全ウェーブを自動プレイ→Run終了(All Clear or Game Over)まで通す自動テストを作成する。
...
- `src/lib/game/shidasu/testHelpers.ts`: `card(id, suit, rank, wild, deckId)`・`ctx(overrides, params)`という共通テストヘルパー。多数のテストファイルから使われている(`ItemEffectContext`の初期値を1箇所で管理する設計)。GUT側でも同等の共通ヘルパーを用意すること。
...
- Godotプロジェクト側: `res://addons/gut/`(GUT本体)、`res://tests/`配下のテストスクリプト一式(vitestの各`*.test.ts`に対応する`test_*.gd`)、共通テストヘルパースクリプト。
- 固定シードによるRun全体の自動プレイスモークテスト(複数シード分)。
- ヘッドレス実行コマンド(READMEやスクリプトとして、どのコマンドで全テストを回せるかを明記)。

## 完了条件

- [ ] GUTがプロジェクトに導入され、コマンドラインからヘッドレスで全テストを実行できる
...
- [ ] `engine.test.ts`の`describe`ブロック一覧と突き合わせ、対応するGUTテストが存在しない項目がないことを確認した
- [ ] 固定シード(複数)によるRun全体の自動プレイスモークテストが、例外を投げずに最後まで完走することを確認した
- [ ] 全テストがGodotエディタを開かずにコマンドラインから実行・合否判定できる

## 注意点

- 「各フェーズ内で都度テストを追加する運用が望ましい」という前提を踏まえ、本フェーズ着手時にまず「フェーズ4〜9のどこまで既にGUTテストがあるか」を棚卸しすることから始める。ゼロから全部書き直す前提で作業しないこと。
- vitestの`describe`/`it`とGUTの`test_`メソッドは1対1対応を基本とし、対応関係が追跡できるようにする(コメントで元のvitestファイル名・行番号を残すなど)。
