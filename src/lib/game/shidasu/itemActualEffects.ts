...
export const ITEM_ACTUAL_EFFECTS: Record<ItemId, string> = {
  ...
  // グループ19: 護符の売値増加(未実装・params定義のみ。Task 3以降で効果実装予定)
  exchange: '(未実装)秘儀・天啓・神託を使用するたび、この護符のsellBonusにnを加算する',
  koban: '(未実装)コンボ数がcに到達した瞬間、この護符のsellBonusにnを加算する',
  senryo: '(未実装)コンボ数がcに到達した瞬間、この護符のsellBonusにnを加算する',
  manryo: '(未実装)コンボ数がcに到達した瞬間、この護符のsellBonusにnを加算する',
  harvest: '(未実装)全消しを達成するたび、この護符のsellBonusにnを加算する',
  settlement: '(未実装)カードプレイc回以下でウェーブをクリアするたび、この護符のsellBonusにnを加算する',
  hiddenTreasure: '(未実装)♠のAをプレイするたび、この護符のsellBonusにnを加算する',
  greatestTreasure: '(未実装)♥のKをプレイするたび、この護符のsellBonusにnを加算する',
  heirloom: '(未実装)♦のJをプレイするたび、この護符のsellBonusにnを加算する',
  treasury: '(未実装)♣のQをプレイするたび、この護符のsellBonusにnを加算する',
  boom: '(未実装)フラッシュが成立するたび、この護符のsellBonusにnを加算する',
  abundantFunds: '(未実装)場札の残り枚数がm枚以下になった瞬間、この護符のsellBonusにnを加算する',
  savings: '(未実装)同じ列を連続でプレイするたび(2回目以降)、連続回数×nをsellBonusに加算する',
  bigCatch: '(未実装)列一掃を達成するたび、この護符のsellBonusにnを加算する',
  grains: '(未実装)ワイルドをプレイするたび、この護符のsellBonusにnを加算する',
  liveliness: '(未実装)同スートが成立し、かつチェーンがm枚以上のとき、この護符のsellBonusにnを加算する',
  prosperity: '(未実装)同色が成立し、かつチェーンがm枚以上のとき、この護符のsellBonusにnを加算する',
  heavenlyBlessing: '(未実装)階段が成立し、かつチェーンがm枚以上のとき、この護符のsellBonusにnを加算する',
  mizuho: '(未実装)交互が成立し、かつチェーンがm枚以上のとき、この護符のsellBonusにnを加算する',
  bountifulYear: '(未実装)ロイヤルセットが成立するたび、この護符のsellBonusにnを加算する',
  profit: '(未実装)同ランクが成立するたび、この護符のsellBonusにnを加算する',
  bounty: '(未実装)コンプリートランが成立するたび、この護符のsellBonusにnを加算する',
  perk: '(未実装)ペアが成立するたび、この護符のsellBonusにnを加算する',
  nestEgg: '(未実装)他の護符を売却するたび、この護符のsellBonusにnを加算する',
}
